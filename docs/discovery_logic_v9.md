...
| `manual_analysis.longitudinal_max_pages` | páginas do userFillsByTime na coleta longitudinal individual | 15 | UPDATE-0059: subiu de 6→15 p/ cobrir mais fundo o histórico não-truncado de hiperativos | só análise individual; scan em massa usa collection.fills_max_pages |
| `manual_analysis.max_requests_per_wallet` | teto informativo de requests HTTP por wallet analisada | 20 | UPDATE-0059: subiu de 12→20 acompanhando longitudinal_max_pages | orçamento por wallet da análise individual |
...
| `manual_analysis.min_sample_days_for_sample_sim` | span mínimo (dias) p/ rodar a simulação AMOSTRAL (`sample_*`) | 1.0 | UPDATE-0059: as `sim_*` longitudinais ficam nulas quando `sampled` (inv. 0056), mas a família paralela `sample_*` simula a cópia sobre o span REALMENTE coberto e reporta "SIM ~$X em Yd" + projeção /30d informativa | preenche `sample_sim_net_usd`/`expectancy`/`max_dd_pct`/`window_days`/`net_per_day`; a guarda anti-sobrescrita não se aplica; abaixo do span os campos ficam nulos. Reclassificação legada (`metrics_confidence` NULL) via `POST /control/discovery/reclassify` (auth de controle) — preserva status/copy_pinned |
...
| `copy_simulation.model_concurrency` | restrição de capital concorrente do espelho | true | notional total aberto ≤ capital × max_copy_leverage; fills que não cabem no orçamento não rendem | UPDATE-0074: corrige SIM NET fantasma de hiperativos de equity minúscula (0xd487 $394 → $826k impossível) sem descartar o trader |
| `copy_simulation.min_funded_share` | fração mínima do book espelhável p/ o SIM NET longitudinal ser confiável | 0.10 | sob concorrência, hiperativos financiam ~0.1% do book com $1000 → ponto-estimativa é ruído (oscila −$1k↔$149k↔$496k) | UPDATE-0074: `funded_share < limiar` → confiança cai p/ `sampled` (âmbar, "cópia parcial"); NÃO descarta — reusa a plumbing de metrics_confidence; `null` desliga o gate |
